...
/** Semana = martes a domingo (6 días). Lunes no se cuenta. */
function getSemanasDesdeMartes(year, month) {
	...
	for (let martes = primerMartes; martes <= lastDate; martes += 7) {
		...
		for (let d = 0; d < 6; d++) {
			...
			if (diaNum <= lastDate) {
				dias.push(`${prefix}${String(diaNum).padStart(2, '0')}`);
			}
		}
...
export default function PedidosView({
	...
	const semanasDe6 = useMemo(() => getSemanasDesdeMartes(year, month), [year, month]);
	const semanaIndexClamped = Math.min(semanaIndex, Math.max(0, semanasDe6.length - 1));
	const semanaActual = semanasDe6[semanaIndexClamped] || [];

	useEffect(() => {
		if (semanaIndex >= semanasDe6.length) setSemanaIndex(Math.max(0, semanasDe6.length - 1));
	}, [semanasDe6.length]);
	...
	const efectivoCaja = Number(efectivoInicial) || 0;
	const efectivoTotalDia = efectivoDia + efectivoCaja;
		...
		<section className="pantalla pedidos-view">
			...
			<div className="pedidos-selector-semana">
				...
				<div className="pedidos-detalle-dia">
					...
					<span className="pedidos-detalle-dia-item">
						Efectivo (ventas del día + efectivo de caja): <strong>{formatMonto(efectivoTotalDia)}</strong>
					</span>
				</div>
			</div>
			...
			<div className="pedidos-detalle">
				...
				<div className="pedidos-detalle-efectivo">
					<span className="pedidos-detalle-label">Efectivo (semana):</span>
